...
int main(int argc, char** argv)
{
	...
	_mkdir((file_path + "photo_zuo").c_str());
	_mkdir((file_path + "photo_you").c_str());
	int relocate = 1;
	while (1)
	{	
		//char* number = new char[50];
		//sprintf(number, "%05d", count);
		string file_name = file_path + to_string(count) + ".jpg";
		//delete number;
		imag1=imread(file_name.c_str());
		if (imag1.empty())
		{
			cout << relocate << endl;
			cerr << "ERROR1" << endl;
		...
		resize(imag1, imageTemp, Size(0, 0), 0.5, 0.5, CV_INTER_LINEAR);
		//imageTemp = imag1;
		...
		if (stage == initial)
		{
			...
			if (face_num > 0)//modefy 0 1-face_num
			{
				...
				int width = (points[1].x - points[0].x)*0.5 + abs(points[1].y - points[0].y)*0.5;
				int height = width;
				...
				string filename_zuo = file_path + "photo_zuo\\" + to_string(count) + "zuo.bmp";//左眼存储目录
				string filename_you = file_path+ "photo_you\\" + to_string(count) + "you.bmp";//右眼存储目录
		...
		{	
			...
			cout << reut.score << endl;
			cout << reut.score_zuo << endl;*/
			if (reut.score > 0.2&&reut.score_zuo > 0.2)
			{
				string filename_zuo = file_path + "photo_zuo\\" + to_string(count) + "zuo.bmp";//左眼存储目录
				string filename_you = file_path + "photo_you\\" + to_string(count) + "you.bmp";//右眼存储目录
				imwrite(filename_zuo.c_str(), roi_zuo);
