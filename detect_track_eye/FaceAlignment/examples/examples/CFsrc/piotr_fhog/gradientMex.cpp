...
// helper for CFgradHist, quantize O and M into O0, O1 and M0, M1 (uses sse)
...
// [Gx,Gy] = CFgrad2(I) - see gradient2.m
void mCFgrad2( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  CFgrad2( I, Gx, Gy, h, w, d );
}

// [M,O] = CFgradMag( I, channel, full ) - see gradientMag.m
void mCFgradMag( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  CFgradMag(I, M, O, h, w, d, full>0 );
}

// CFCFgradMagNorm( M, S, norm ) - operates on M - see gradientMag.m
void mCFCFgradMagNorm( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  CFCFgradMagNorm(M,S,h,w,norm);
}

// H=CFgradHist(M,O,[...]) - see gradientHist.m
void mCFgradHist( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  if( useHog==0 ) {
    CFgradHist( M, O, H, h, w, binSize, nOrients, softBin, full );
  } else if(useHog==1) {
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  else if(!strcmp(action,"gradient2")) mCFgrad2(nl,pl,nr,pr);
  else if(!strcmp(action,"gradientMag")) mCFgradMag(nl,pl,nr,pr);
  else if(!strcmp(action,"gradientMagNorm")) mCFCFgradMagNorm(nl,pl,nr,pr);
  else if(!strcmp(action,"gradientHist")) mCFgradHist(nl,pl,nr,pr);
