...
{
public:
    VOT(const std::string & region_file)
    {
        //_images = images;
    ...
    inline cv::Rect getInitRectangle()  
    {   
        // read init box from ground truth file
        VOTPolygon initPolygon = getPolygon();
    ...
    {   
		...
		if (p_region_stream.is_open()){
			...
			if (numbers.size() == 4) {
				...
				std::cerr << "Error loading initial region in file - unknow format " << "!" << std::endl;//region_file <<
				p.x1 = 0;
				p.y1 = 0;
				p.x2 = 0;
				p.y2 = 0;
			...
			std::cerr << "Error loading initial region in file " <<  "!" << std::endl;//region_file <<
			p.x1 = 0;
			p.y1 = 0;
			p.x2 = 0;
			p.y2 = 0;
